          ...
          withCompletionHandler:(void (^)(void))handler
{
    ...
    if ([mutableUserInfo objectForKey:@"ab"]) {
        BrazePlugin *pluginInstance = (BrazePlugin *)[(CDVViewController *)self.viewController
                                       getCommandInstance:@"BrazePlugin"];
        // Call Braze’s built-in method on the `.notifications` property:
        [pluginInstance.braze.notifications handleUserNotificationWithResponse:response
